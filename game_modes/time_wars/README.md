...
## Робоча модель гри (таймер зверху)

Один HTML-файл з таймером зверху та картками гравців; можна подивитись модель гри без запуску агентів:

- **Відкрити в браузері:** `game_modes/time_wars/demo_board.html` (подвійний клік або File → Open).
- **Або з консолі:** `python open_demo_board.py` — відкриє дошку в браузері.

На сторінці: таймер гри (тік X / Y, прогрес-бар, «Залишилось»), гравці (ім’я, роль, час у сек). Кнопка **«Запустити демо»** прогоняє коротку симуляцію (тіки, шторм, криза, кооп/крадіжка) щоб побачити оновлення в реальному часі.
